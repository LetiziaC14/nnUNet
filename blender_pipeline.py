...
def execute_blender_pipeline():
    ...
    try:
        ...
        print(
            "ERRORE CRITICO: Impossibile importare un modulo fondamentale. "
            "Controlla che le librerie necessarie (es. PyYAML) siano "
            f"installate nell'ambiente Python di Blender. Dettagli: {e}"
        )
        sys.exit(1)
    ...
    try:
        ...
        print(
            "ERRORE: File fondamentale non trovato: {e}. "
            "Assicurati che la pipeline di segmentazione sia stata eseguita correttamente."
        )
        return
    except Exception as e:
    ...
    enriched_manifest = blender_ops.match_materials_on_manifest(
        segments_manifest, blender_shader_registry
    )
    ...
    if not os.path.exists(config.INPUT_MESH_DIR) or not os.listdir(config.INPUT_MESH_DIR):
        print(
            f"ERRORE: La directory di input '{config.INPUT_MESH_DIR}' non esiste o e' vuota. "
            "Nessun mesh da processare."
        )
        return
    ...
    print(
        "\n--- Fase 5: Applicazione unita' in scala reale, fattore di conversione: "
        f"'{config.WORLD_SCALE_FACTOR}' ---"
    )
    ...
    scene_root = blender_ops.create_single_scene_root(
        imported_meshes, config.ROOT_NAME_BASE
    )
    if not scene_root:
    ...
    blender_ops.merge_vertices_by_distance(
        imported_meshes, config.MERGE_DISTANCE
    )  # disconnected faces
    polycount, poly_removed = blender_ops.decimate_mesh_objects(
        imported_meshes, config.MAX_FACES_PER_MESH, segments_manifest
    )  # decimation
    print(f"RECAP DECIMATION: Total: '{polycount}', Removed: '{poly_removed}'")
    blender_ops.delete_small_features(
        imported_meshes, config.MERGE_DISTANCE
    )  # dissolve_degenerate to fix potential decimation leftovers
    blender_ops.apply_smoothing_normals(
        imported_meshes, config.NORMAL_SMOOTHING_METHOD
    )  # smoth
    ...
    template_nodes = blender_ops.apply_materials_from_manifest(
        imported_meshes, enriched_manifest
    )
    ...
    blender_ops.save_blender_scene(
        config.OUTPUT_DIR, f"{config.PROJECT_SESSION_ID}_01_history.blend"
    )
    ...
    blender_ops.create_base_metalness_map(
        imported_meshes, config.TEXTURES_DIR, config.TEXTURE_SIZE
    )
    ...
    blender_ops.save_blender_scene(
        config.OUTPUT_DIR, f"{config.PROJECT_SESSION_ID}_02_baked_PBR.blend"
    )
    ...
    blender_ops.create_metallic_smoothness_map(
        imported_meshes, config.TEXTURES_DIR, config.TEXTURE_SIZE
    )

    # --- 19 Collega le texture al materiale (Unity URP Standard)
    print("\n--- Fase 19: Collegamento nodi texture in standard URP")
    urp_nodes_created = blender_ops.update_shader_nodes_for_unity_export(
        imported_meshes, config.TEXTURES_DIR
    )
    ...
    blender_ops.save_blender_scene(
        config.OUTPUT_DIR, f"{config.PROJECT_SESSION_ID}_02_baked_URP.blend"
    )
...
if __name__ == "__main__":
    ...
    if is_inside_blender:
        ...
        try:
            ...
            print(
                "ERRORE CRITICO: Impossibile creare le directory necessarie. Dettagli: {e}"
            )
            sys.exit(1)
        ...
        try:
            utils.yaml_to_json(
                config.BLENDER_SHADER_REGISTRY_FILE, config.BLENDER_SHADER_REGISTRY_TMP
            )
        except Exception as e:
            print(
                "ERRORE CRITICO: Impossibile creare il file di registro JSON. "
                "Interruzione. Dettagli: {e}"
            )
            sys.exit(1)
        ...
        try:
            ...
            print(
                "ERRORE: L'eseguibile di Blender non è stato trovato in "
                f"'{blender_executable}'. Controlla il percorso in config.py."
            )
            sys.exit(1)
