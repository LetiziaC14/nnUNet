...
import json
import config  # this is fine at top-level, Blender can import config

def read_yaml(file):
    # Local import so Blender doesn’t need yaml unless this function is used
    import yaml
...
def yaml_to_json(yaml_file_path, json_file_path):
    ...
    """
    # Also safe to rely on read_yaml (which already imports yaml locally)
    try:
